...
export const TeachingDialog: React.FC<TeachingDialogProps> = ({
  isOpen,
  onClose,
  onSuccess
}) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success('Nastava je uspješno kreirana');
      onSuccess?.();
      onClose();
    } catch (err) {
      console.error('Error creating teaching:', err);
      setError('Došlo je do greške pri kreiranju nastave');
    } finally {
      setIsLoading(false);
    }
  };
            ...
            <div className="relative bg-slate-800/90 backdrop-blur-xl rounded-lg shadow-xl border border-white/10">
              <div className="flex items-center justify-between p-6 border-b border-white/10">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <h2 className="text-xl font-semibold text-white">Nova nastava</h2>
                    <p className="text-sm text-gray-400">Dodajte novu nastavu</p>
                  </div>
                </div>
                {!isLoading && (
                  <motion.button
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-6">
                ...
                <div className="flex justify-end space-x-4">
                  ...
                  <button
                    type="submit"
                    disabled={isLoading || !formData.class_id || !formData.subject_id || !formData.teacher_id}
